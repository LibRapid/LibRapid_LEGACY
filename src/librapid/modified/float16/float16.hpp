...
namespace librapid::extended {
	...
	struct float16_t {
		...
		inline operator T() const noexcept {
			...
			return static_cast<T>(f32.float_);
		}
...
#undef NUM_LIM
	...
	LR_INLINE extended::float16_t sin(extended::float16_t x) noexcept { return extended::sin(x); }
	LR_INLINE extended::float16_t cos(extended::float16_t x) noexcept { return extended::cos(x); }
	LR_INLINE extended::float16_t tan(extended::float16_t x) noexcept { return extended::tan(x); }
